#!/usr/bin/env python
"""Generate multiclass histogram LLP baseline jobs."""
...
def main() -> None:
    ...
    parser.add_argument("--out-root", default="configs/histogram_llp")
    ...
    parser.add_argument("--losses", nargs="+", default=["kl", "mse", "ce"])
        ...
        for train_bags in args.train_bags:
            for seed in args.seeds:
                ...
                for loss in args.losses:
                    name = f"{args.dataset.lower()}_histogram_llp_{loss}_n{bag_mean}_train{train_bags}_s{seed}"
                    cfg = out_dir / "histogram_llp" / f"{name}.yaml"
                    _write(cfg, f"experiment: mnist_histogram_llp\nloss: {loss}\n" + common)
