#!/usr/bin/env python
"""Generate expected-sum baseline jobs for digit-sum bags."""
...
def main() -> None:
    ...
    parser.add_argument("--out-root", default="configs/digit_sum_baseline")
    ...
    parser.add_argument("--objectives", nargs="+", default=["mse", "huber", "mae"])
        ...
        for train_bags in args.train_bags:
            for seed in args.seeds:
                ...
                for objective in args.objectives:
                    name = f"{args.dataset.lower()}_digit_sum_baseline_{objective}_n{bag_mean}_train{train_bags}_s{seed}"
                    cfg = out_dir / "digit_sum_baseline" / f"{name}.yaml"
                    _write(cfg, f"experiment: mnist_digit_sum_baseline\nobjective: {objective}\n" + common)
                    rows.append(f"{name}\tscripts/train_mnist_digit_sum_baseline.py\t{cfg}\t\n")
